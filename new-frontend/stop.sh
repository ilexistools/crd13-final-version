...
stop_from_pid_file() {
  ...
  if [[ "$pid" =~ ^[0-9]+$ ]]; then
    if kill -0 "$pid" >/dev/null 2>&1; then
      ...
      echo "Frontend process from PID file is not running."
    fi
  fi
...
stop_listeners_on_port() {
  ...
  for pid in $(lsof -tiTCP:"$port" -sTCP:LISTEN 2>/dev/null || true); do
    ...
    if [[ "$command_line" != *"$ROOT"* ]]; then
      ...
      echo "Skipping listener on port $port (PID $pid, ${process_name:-unknown}); it was not started from this frontend."
      continue
    fi
