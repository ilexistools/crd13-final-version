...
export class PageReviewer implements OnInit {
  ...
  requirementsSidebarWidth = 340;
  resizingRequirementsSidebar = false;
  private readonly requirementsSidebarMinWidth = 72;
  private readonly requirementsSidebarDefaultWidth = 340;
  private readonly requirementsSidebarMaxWidth = 520;
  private requirementsSidebarExpandedWidth = this.requirementsSidebarDefaultWidth;
  ...
  get isRequirementsSidebarCompact(): boolean {
    return this.reviewLayoutMode === 'requirements_sidebar' && this.requirementsSidebarWidth <= 140;
  }
  ...
  startRequirementsSidebarResize(event: MouseEvent): void {
    if (this.reviewLayoutMode !== 'requirements_sidebar') return;

    event.preventDefault();
    this.resizingRequirementsSidebar = true;
  }

  resetRequirementsSidebarWidth(): void {
    this.requirementsSidebarWidth = this.requirementsSidebarDefaultWidth;
    this.requirementsSidebarExpandedWidth = this.requirementsSidebarDefaultWidth;
  }

  toggleRequirementsSidebarCompact(): void {
    if (this.isRequirementsSidebarCompact) {
      this.requirementsSidebarWidth = this.requirementsSidebarExpandedWidth;
      return;
    }

    this.requirementsSidebarExpandedWidth = Math.max(
      this.requirementsSidebarWidth,
      this.requirementsSidebarDefaultWidth
    );
    this.requirementsSidebarWidth = this.requirementsSidebarMinWidth;
  }

  @HostListener('document:mousemove', ['$event'])
  onRequirementsSidebarResize(event: MouseEvent): void {
    if (!this.resizingRequirementsSidebar) return;

    const nextWidth = Math.round(event.clientX);
    this.requirementsSidebarWidth = Math.min(
      this.requirementsSidebarMaxWidth,
      Math.max(this.requirementsSidebarMinWidth, nextWidth)
    );

    if (!this.isRequirementsSidebarCompact) {
      this.requirementsSidebarExpandedWidth = this.requirementsSidebarWidth;
    }
  }

  @HostListener('document:mouseup')
  stopRequirementsSidebarResize(): void {
    this.resizingRequirementsSidebar = false;
  }
  ...
  confirmDeleteComponent(): void {
    ...
    this.selectedTemplate.structural_template = this.removeComponentFromStructuralTemplate(
      this.selectedTemplate.structural_template || '',
      found
    );

    // Remove from components object
    this.removeComponentMetadata(found);
  ...
  private removeComponentFromStructuralTemplate(template: string, componentName: string): string {
    const escaped = this.escapeRegExp(componentName);
    const componentPattern = `\\*?<${escaped}>`;
    let updatedTemplate = template || '';

    // If the component is part of an optional phrase, remove the whole phrase.
    // Example: "{[by] <AuthorityOrAgent>}" should disappear as a unit.
    updatedTemplate = updatedTemplate.replace(
      new RegExp(`\\s*\\{[^{}]*${componentPattern}[^{}]*\\}\\s*`, 'gi'),
      ' '
    );

    // Common non-braced tails generated by the adapter.
    const phrasePatterns = [
      `\\s*in\\s+accordance\\s+with\\s+${componentPattern}`,
      `\\s*(?:according|pursuant|subject)\\s+to\\s+${componentPattern}`,
      `\\s*(?:under|by|with)\\s+${componentPattern}`,
    ];

    for (const pattern of phrasePatterns) {
      updatedTemplate = updatedTemplate.replace(new RegExp(pattern, 'gi'), ' ');
    }

    updatedTemplate = updatedTemplate.replace(new RegExp(`\\s*${componentPattern}\\s*`, 'gi'), ' ');
    return this.normalizeStructuralTemplate(updatedTemplate);
  }

  private normalizeStructuralTemplate(template: string): string {
  ...
  private removeComponentMetadata(componentName: string): void {
    if (!this.selectedTemplate?.components) return;

    for (const [key, component] of Object.entries(this.selectedTemplate.components)) {
      if (component?.label === componentName) {
        delete this.selectedTemplate.components[key];
      }
    }
  }
