...
usage() {
  cat <<EOF
Usage: ./start.sh [--backend-port PORT] [--frontend-port PORT] [--no-install]
EOF
}
...
stop_stale_process "Frontend" "$RUNTIME_DIR/frontend.pid"

if port_in_use "$BACKEND_PORT"; then
  echo "Port $BACKEND_PORT is already in use. Run ./stop.sh or choose another backend port." >&2
  exit 1
fi

if port_in_use "$FRONTEND_PORT"; then
  echo "Port $FRONTEND_PORT is already in use. Run ./stop.sh or choose another frontend port." >&2
  exit 1
fi
